  ...
  return (
    <div className="min-h-screen flex flex-col text-[var(--color-nike-white)]" style={{ backgroundColor: 'var(--color-nike-black)', color: 'var(--color-nike-white)' }}>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-1 overflow-x-auto scrollbar-none">
              {['vendor', 'coach', 'gym_owner'].includes(user?.role) ? (
                <>
                  {[{ to: '/', label: 'Home' }, { to: '/vendor', label: 'Dashboard' }, { to: '/premium', label: 'Premium' }, ...(user?.role === 'admin' ? [{ to: '/admin', label: 'Admin' }] : [])].map((link) => (
                    <Link key={link.to} to={link.to} onClick={() => playWhoosh()} className={'relative text-sm tracking-widest uppercase font-medium px-3 xl:px-4 py-2 rounded-full whitespace-nowrap transition-all duration-300 group ' + (isLight ? 'hover:bg-nike-gray/30 text-nike-black' : 'hover:bg-white/5')} style={!isLight ? { color: 'var(--color-nike-light)' } : {}}>
                      {link.label}
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={'lg:hidden relative p-2 rounded-xl transition-all duration-200 ' + (isLight ? 'hover:bg-nike-gray/30' : 'hover:bg-white/5')}
      ...
      <div className={'fixed inset-0 z-40 transition-opacity duration-300 lg:hidden ' + (mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}>
        ...
        <div className={'absolute top-16 left-0 w-72 h-[calc(100vh-4rem)] overflow-y-auto border-r backdrop-blur-md transition-transform duration-300 ' + (mobileOpen ? 'translate-x-0' : '-translate-x-full') + ' ' + (isLight ? 'bg-white/95 border-nike-gray' : '')} style={!isLight ? { backgroundColor: 'color-mix(in srgb, var(--color-nike-dark) 98%, transparent)', borderColor: 'var(--color-nike-gray)' } : {}}>
          <div className="p-4 space-y-1">
            {(user ? [
              ...
              ...(user.role === 'coach' ? [{ to: '/coach', label: 'Dashboard', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' }] : []),
              ...(['vendor', 'coach', 'gym_owner'].includes(user.role) ? [{ to: '/vendor', label: 'My Dashboard', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' }] : []),
